...
function buildBuckets(orders: Order[], period: Period): TaxBucket[] {
  const getKey =
  ...
  for (const [key, bucketOrders] of map) {
    const taxCollected = bucketOrders.reduce((sum, o) => sum + (o.tax || 0), 0);
...
function exportTaxCSV(buckets: TaxBucket[]): void {
  const headers = ['Period', 'Taxable Sales', 'Tax Collected'];

  const rows = buckets.map((b) => [b.label, b.totalRevenue.toFixed(2), b.taxCollected.toFixed(2)]);
...
export default function TaxReportPage() {
  ...
  const orders = useLiveQuery(
    () =>
      db.orders
        .where('shopId')
        .equals(activeShopId ?? '')
        .filter((o) => o.status === 'paid' && (o.tax ?? 0) > 0)
        .toArray(),
    [activeShopId],
  );
  ...
  const totals = useMemo(() => {
    ...
    return buckets.reduce(
      (acc, b) => ({
        taxCollected: acc.taxCollected + b.taxCollected,
        orderCount: acc.orderCount + b.orderCount,
      }),
      { taxCollected: 0, orderCount: 0 },
    );
  }, [buckets]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Tax Report</h1>
          <p className="text-sm text-gray-500">
            Total tax collected per{' '}
      ...
      {buckets.length > 0 && (
        <div className="flex gap-3">
          ...
          <div className="flex-1 rounded-lg border border-gray-200 bg-white px-3 py-2">
            ...
            <div className="text-lg font-bold text-blue-700">{totals.orderCount}</div>
          </div>
        </div>
      )}
      ...
      {buckets.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-gray-500">
          ...
          <p className="mt-1 text-xs text-center">
            Tax collected will appear here once orders are paid.
            <br />
      ...
      {buckets.length > 0 && (
        <div className="rounded-lg border border-gray-200 bg-white overflow-hidden">
          ...
          {buckets.map((bucket) => {
            ...
            const barWidth = Math.max((bucket.taxCollected / maxTax) * 100, 2);
              ...
              <div key={bucket.key}>
                ...
                >
                  ...
                  <span className="w-20 text-right text-sm font-bold tabular-nums text-green-700">
                    ${bucket.taxCollected.toFixed(2)}
                  </span>
                  <span className="w-12 text-right text-xs text-gray-500">{bucket.orderCount}</span>
      ...
      <div className="flex items-center justify-center gap-2 pt-2 text-xs text-gray-500">
        <span>
          Tax report for {buckets.length}{' '}
          {period === 'monthly' ? 'months' : period === 'quarterly' ? 'quarters' : 'years'}
        </span>
        <span>·</span>
        <button
