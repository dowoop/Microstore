...
export interface LicensePayload {
  tier: 'pro'; shopId: string; licensee?: string; iat: number; exp: number;
}
...
export async function verifyAndActivateLicense(key: string): Promise<LicenseValidationResult> {
  const dot = key.indexOf('.');
  if (dot <= 0 || dot === key.length - 1) return { valid: false, error: 'Invalid format' };

  let payloadBytes: Uint8Array, sigBytes: Uint8Array;
  try { payloadBytes = b64urlDecode(key.slice(0, dot)); sigBytes = b64urlDecode(key.slice(dot + 1)); }
  ...
  try { rawSig = derToRaw(sigBytes); }
  catch { return { valid: false, error: 'Invalid signature format' }; }

  try {
    const pubKey = await getPublicKey();
    const valid = await crypto.subtle.verify(
      { name: 'ECDSA', hash: 'SHA-256' }, pubKey, rawSig.buffer as ArrayBuffer, payloadBytes.buffer as ArrayBuffer);
    if (!valid) return { valid: false, error: 'Invalid signature' };
  } catch (err) {
    return { valid: false, error: 'Verify failed: ' + (err instanceof Error ? err.message : '') };
  }
