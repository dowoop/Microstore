...
export interface TokenPickerProps {
  /** Currently selected tokens (mutable). */
  selected: AcceptedToken[];
  /** Called when a token is added. */
  onAdd: (token: AcceptedToken) => void;
  /** Called when a token is removed (by mint). */
  onRemove: (mint: string) => void;
  /** Called when tokens are reordered. */
  onReorder: (fromIndex: number, toIndex: number) => void;
  /** Solana cluster for RPC calls. */
  cluster?: string;
}
    ...
    <div ref={containerRef} className="space-y-3">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {isKnown?.verified && (
                      <ShieldCheck className="h-3.5 w-3.5 text-blue-500" title="Verified token" />
                    )}
                  </div>
                ...
                <button
                  ...
                  className="shrink-0 rounded-full p-1 text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors"
                  title="Remove token"
                >
                  <X className="h-4 w-4" />
                </button>
      ...
      <div className="relative">
        <button
          ...
          className="flex w-full items-center justify-between gap-2 rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-500 hover:border-purple-400 hover:text-purple-700 transition-colors"
        >
              ...
              {filtered.map((token) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-left hover:bg-purple-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
            ...
            <div className="border-t border-gray-100">
              {showCustom ? (
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-gray-500 hover:bg-gray-50 transition-colors"
                >
...
// ---------------------------------------------------------------------------
// CustomTokenEntry — inline mint validation + add
// ---------------------------------------------------------------------------
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setValid(false);
        setError('Failed to validate mint on-chain. Check your connection.');
      } finally {
        setValidating(false);
      }
    }, 600);
    ...
    <div className="px-4 py-3 space-y-2">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex-1 rounded-md bg-purple-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-purple-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          Add token
        </button>
        <button
